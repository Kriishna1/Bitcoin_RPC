...
// hash256: double SHA256 that returns a hex string.
...
// Compute wtxid: for a full transaction hex, compute double SHA256 and then reverse byte order.
...
// Create coinbase transaction with a provided witness commitment.
// Initially, the witness is empty.
function createCoinbaseTxWithCommitment(commitment) {
  ...
  // Script: OP_RETURN (0x6a) || OP_PUSHBYTES_36 (0x24) || prefix (0xaa21a9ed) || [commitment]
  ...
  // Initially, set the witness stack to empty.
...
// Update the coinbase transaction: insert valid witness commitment output and set witness stack.
function updateCoinbaseTx(commitment) {
  ...
  const locktime = intToLE(0, 4);
  // Set witness stack to contain the reserved value (32 zeros)
...
// For coinbase, per BIP141, the wtxid is defined as 32 zero bytes.
