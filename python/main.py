...
def create_coinbase_tx_with_commitment(commitment: bytes) -> (str, str, str):
    ...
    # Build script: OP_RETURN (0x6a) || OP_PUSH36 (0x24) || prefix || commitment
...
# For coinbase, use 32 zero bytes.
coinbase_wtxid = "00" * 32
wtxids = [coinbase_wtxid]
for tx in selected_mempool:
    full_hex = tx["hex"]
    if full_hex:
        # Compute wtxid = double_sha256(full_hex) then reverse byte order.
...
# Step 3: Witness commitment input = witness_merkle_root (as bytes) concatenated with 32 zero bytes.
witness_commitment_input = bytes.fromhex(witness_merkle_root) + b'\x00' * 32
# Step 4: Compute witness commitment = double_sha256(witness_commitment_input)
